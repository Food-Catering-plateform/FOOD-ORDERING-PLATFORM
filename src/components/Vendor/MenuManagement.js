...
// Placeholder used by the component when no image is provided on add
const PLACEHOLDER_IMAGE = 'https://placehold.co/300x160/F5F4F2/FF6B2B?text=Food';
...
describe('MenuManagement – adding an item', () => {
  it('calls addDoc with correct data and updates the list', async () => {
    ...
    // Placeholders match the component: 'Food name', 'Price (R)', 'Quantity available', 'Description'
  ...
  it('uses a placeholder image when no file is selected', async () => {
    ...
    expect(addDoc).toHaveBeenCalledWith(
      'menu-collection-ref',
      expect.objectContaining({ imageUrl: PLACEHOLDER_IMAGE })
    );
  });
});
...
describe('MenuManagement – Firestore integration details', () => {
  ...
  it('trims whitespace from name and description before saving', async () => {
    ...
    // userEvent.type does not reliably produce leading/trailing spaces,
    // so we fire a change event directly to set the raw value with spaces.
    const nameInput = screen.getByPlaceholderText('Food name');
    ...
    fireEvent.change(nameInput, { target: { value: '  Wrap  ' } });
    fireEvent.change(descInput, { target: { value: '  Tasty  ' } });
